...
public class DoubleLinkedList<T extends Serializable> implements LinkedListInterface<T>, Cloneable, Serializable {
        ...
        if (!isEmpty()) {
            try {
                ...
                if (nodeOf(getin) == head) {
                    ...
                    DoubleListNode<T> nodePrev = getPreviousNode((NodeInterface<T>) node1);
                    DoubleListNode<T> newNode = new DoubleListNode<T>(obj);
                    add((NodeInterface<T>) nodePrev, (T) newNode);
                    remove((T) node1);
                    return true;
                }
            } catch (Exception ex) {
    ...
    public NodeInterface<T> nodeOf(T object) {
        ...
        if (object != null) {
            try {
                while (iter.hasNext()) {
                    DoubleListNode newNode = (DoubleListNode) iter.next();
                    if (newNode.isEquals(object)) {
                        return (NodeInterface<T>) newNode;
                    }
                }
            } catch (Exception ex) {
                Logger.getLogger(this.getClass().getName()).log(Level.WARNING, ex.getMessage(), ex);
        ...
        return (NodeInterface<T>) new DoubleListNode<>(null);
    }
    ...
    @Override
    public T[] toArray() {
        NodeInterface<T> nodo = (NodeInterface<T>) head;
    ...
    public Iterator<NodeInterface<T>> iterator() {
        ...
        return new Iterator<NodeInterface<T>>() {
            ...
            public NodeInterface<T> next() {
                ...
                NodeInterface<T> node = (NodeInterface<T>) inode;
                inode = inode.getNext();
                return node;
            }
        };
    ...
    public boolean invertir(int k) {
        if (k > 0 && !isEmpty()) {
            ...
            for (int i = 0; i < tamaño; i++) {
                ...
                if (size() == k) {
                    nodeFor = getPreviousNode((NodeInterface<T>) kNode);
                ...
                if (kNode.isEquals(tail.getObject())) {
                    if (k == 2) {
                        prevNode = getPreviousNode((NodeInterface<T>) kNode);
                        moveNode = getPreviousNode((NodeInterface<T>) firstNode);
                ...
                if (firstNode.isEquals(head.getObject())) {
                    if (k == 2) {
                        ...
                        moveNode = getPreviousNode((NodeInterface<T>) kNode);
